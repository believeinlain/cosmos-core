...
void sph_sim::kernel_type() {
	...
	assignd_by_index(this->prop.kernel_type, I, 1);
...
tuple<MatrixXd, Matrix3D, Matrix3D> sph_sim::sph_compute_dij() {
	...
	// NOTE: Check this
	MatrixXd I = unit_ij.z0.array().isInf().cast<double>();
	assignd_by_index(unit_ij.z0, find(I), 0);
	I = unit_ij.z1.array().isInf().cast<double>();
	assignd_by_index(unit_ij.z1, find(I), 0);
	I = unit_ij.z2.array().isInf().cast<double>();
	assignd_by_index(unit_ij.z2, find(I), 0);
	...
	for(int i = 0; i < this->nrd; ++i) {
		...
		MatrixXi I2 = find( ( this->prop.group.array() != this->prop.group(I1) )
								.select(MatrixXd::Ones(this->prop.group.rows(),this->prop.group.cols()),
										MatrixXd::Zero(this->prop.group.rows(),this->prop.group.rows())) );
		//M(I2,I1)=0; // NOTE: INCOMPLETE: what is this?
	}

	MatrixXi I = find( ( M.array() != 0 )
								.select(MatrixXd::Ones(M.rows(),M.cols()),
										MatrixXd::Zero(M.rows(),M.rows())) );
	
	return make_tuple(M,I);
}

void sph_sim::sph_compute_density(const MatrixXd& dij, const MatrixXd& Mask, const MatrixXi& MaskI) {
	// Reshape mask vector into matrix
	MatrixXd mj = Mask * this->prop.m.transpose(); // NOTE: MODIFIED, what is the purpose of the .*(ones(obj.npart,1) ?
}
